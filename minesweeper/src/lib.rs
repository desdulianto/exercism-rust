fn count_mines(i: usize, j: usize, minefield: &[&str]) -> usize {
    let (rows, cols) = (minefield.len(), if !minefield[i].is_empty() { minefield[0].len() } else { 0 });
    ...
    [
     ...
     .map(|pos| {
            ...
            if Some('x') == minefield[x].chars().nth(y) {
                1
            } else {
                0
            }
...
pub fn annotate(minefield: &[&str]) -> Vec<String> {
    minefield.iter().enumerate().map(|r| {
        let (i, row) = r;
        row.chars().enumerate().map(|c| {
            match c {
                (_, '*') => String::from("*"),
                (col, _) => {
